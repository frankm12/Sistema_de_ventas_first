    ...
    public partial class Consulta_producto : Form
    {
        ...
        private void bt_editar_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                idProducto = int.Parse(row.Cells["id_producto"].Value.ToString());
                nombre = (row.Cells["nombreProducto"].Value.ToString());
                idLinea = row.Cells["id_lineaProducto"].Value.ToString();
                cantidad = int.Parse(row.Cells["cantidad"].Value.ToString());
                precio = decimal.Parse(row.Cells["precioVenta"].Value.ToString());
